...
int main(){
	...
	auto start = std::chrono::high_resolution_clock::now();//start pega início da execução do loop FOR
	/*Medição de tempo - steady_clock */
//	steady_clock::time_point t1 = steady_clock::now();

/** Teste de tempo de execução - Loop "FOR" */
int i;
for(i=0;i<150000;i++){
...
//			obj2->getNumRodas();
	...
	Veiculo * veiculo = new Veiculo("<Veiculo carro>");
	veiculo->getNumRodas();
	delete veiculo;
...
//	cout<< "Tempo de trabalho: " << tempo.count()<<" segundos"<<endl;
	...
	auto finish = std::chrono::high_resolution_clock::now(); //Finish pega o tempo depois de execução do código
 ...
 *
 *
 * auto start = std::chrono::high_resolution_clock::now();//start pega início da execução do código
 *
 * // Aqui fica o código
 *
 * auto finish = std::chrono::high_resolution_clock::now(); //Finish pega o tempo depois de execução do código
